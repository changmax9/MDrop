MDrop 0.2.3 — Ad-hoc Signed Build

This build uses a local ad-hoc signature and is not notarized. macOS may block
its first launch.

To open it:
1. Move MDrop.app to Applications.
2. Try to open it once.
3. Open System Settings → Privacy & Security.
4. Select “Open Anyway” for MDrop.

Only use builds whose SHA-256 matches the accompanying checksum file.

—

MDrop 0.2.3 — 本地签名、未公证版本

此版本使用本地 ad-hoc 签名，未经过 Apple 公证，因此 macOS 首次运行时可能会
阻止打开。

打开方法：
1. 将 MDrop.app 移入“应用程序”。
2. 尝试打开一次。
3. 打开“系统设置 → 隐私与安全性”。
4. 在 MDrop 提示旁选择“仍要打开”。

请核对随附 SHA-256 校验值后再使用。
